...
export default function AddPostPage() {
    ...
    if (storedUser) {
      try {
        ...
        setModalState({
          ...
          message: "Silakan login kembali untuk membuat post.",
          type: "error",
          buttons: [
            {
              text: "Login",
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      const validFiles = filesToUpload.filter((file) => {
        if (file.size > 5 * 1024 * 1024) {
          ...
          setModalState({
            ...
            message: `File "${file.name}" melebihi batas 5MB.`,
            type: "warning",
      ...
      const newImagesToAdd = validFiles.slice(
        0,
        Math.min(5 - currentImagesCount, validFiles.length),
      );
      if (
        currentImagesCount + newImagesToAdd.length > 5 &&
        validFiles.length > 0
      ) {
        setModalState({
          isOpen: true,
          title: "Batas Maksimal Gambar",
          message: "Anda hanya bisa mengunggah total 5 gambar.",
          type: "warning",
          buttons: [{ text: "OK", onClick: closeGenericModal }],
          onClose: closeGenericModal,
        });
        ...
        setImages((prevImages) => [
          ...prevImages,
          ...newImagesToAdd.slice(0, spaceLeft),
        ]);
        return;
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!author.trim())
      validationMessage += "Nama author tidak boleh kosong.\n";
    if (!category.trim())
      validationMessage += "Kategori karya tidak boleh kosong.\n";
    if (!description.trim())
      validationMessage += "Deskripsi karya tidak boleh kosong.\n";
    if (images.length === 0)
      validationMessage += "Mohon unggah setidaknya satu gambar.\n";
    ...
    if (validationMessage) {
      setModalState({
        ...
        type: "warning",
        buttons: [{ text: "Saya Mengerti", onClick: closeGenericModal }],
        onClose: closeGenericModal,
      });
      return;
    }
    ...
    try {
      for (const imageFile of images) {
        ...
        if (!response.ok) {
          const errorData = await response
            .json()
            .catch(() => ({ error: `Gagal unggah ${imageFile.name}` }));
        ...
        if (result.fileUrl) {
          ...
          throw new Error(
            `URL file tidak ada di respons API untuk ${imageFile.name}.`,
          );
        }
      }
      ...
      setModalState({
        ...
        message:
          uploadError.message || "Terjadi kesalahan saat mengunggah gambar.",
        type: "error",
        buttons: [{ text: "OK", onClick: closeGenericModal }],
        onClose: closeGenericModal,
      });
    ...
    try {
      const existingPosts = JSON.parse(
        localStorage.getItem("designerPosts") || "[]",
      );
      ...
      setModalState({
        ...
        message: "Karya Anda telah berhasil ditambahkan ke portofolio.",
        type: "success",
        buttons: [
          {
      ...
      setModalState({
        ...
        message: "Terjadi kesalahan saat menyimpan karya Anda.",
        type: "error",
        buttons: [{ text: "Coba Lagi", onClick: closeGenericModal }],
        onClose: closeGenericModal,
      });
    } finally {
      setIsSubmitting(false);
        ...
        <div className="relative z-10 flex flex-col lg:flex-row min-h-screen">
          <div className="lg:w-1/2 flex flex-col items-center justify-center p-8 lg:p-16 text-gray-800">
            <div className="absolute top-6 left-6 z-20 lg:hidden">
              <button
                onClick={() => router.back()}
                className="flex items-center gap-2 text-emerald-700 hover:text-emerald-800 transition-colors bg-white/50 backdrop-blur-sm p-2 rounded-full shadow"
              >
            ...
            <div className="text-center lg:text-left max-w-lg mt-16 lg:mt-0">
              <div className="mb-8">
                ...
                <p className="text-lg sm:text-xl text-gray-600 leading-relaxed">
                  Ini adalah panggungmu! Unggah karya desain, ilustrasi, atau
                  proyek kreatif lainnya dan biarkan dunia melihat bakatmu.
                </p>
              </div>
              <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-start space-y-4 sm:space-y-0 sm:space-x-6 text-gray-700">
                <div className="flex items-center">
                  {" "}
                  <div className="w-10 h-10 bg-gradient-to-br from-emerald-400 to-teal-500 rounded-full flex items-center justify-center mr-3 shadow">
                    {" "}
                    <UploadCloudIcon className="w-5 h-5 text-white" />{" "}
                  </div>{" "}
                  <span>Unggah Mudah & Cepat</span>{" "}
                </div>
                <div className="flex items-center">
                  {" "}
                  <div className="w-10 h-10 bg-gradient-to-br from-sky-400 to-blue-500 rounded-full flex items-center justify-center mr-3 shadow">
                    {" "}
                    <ImageIcon className="w-5 h-5 text-white" />{" "}
                  </div>{" "}
                  <span>Bangun Portofolio</span>{" "}
                </div>
              </div>
            </div>
          </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        placeholder="Contoh: Ilustrasi Pemandangan Senja"
                        required
                      />
                    </div>
                  </div>
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          className="w-full pl-10 pr-3 py-3 bg-gray-50 border border-gray-300 rounded-lg focus:bg-white focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition-colors placeholder-gray-400 text-sm"
                          placeholder="Misal: Ilustrasi, Desain Karakter"
                          required
                        />
                      </div>
                    </div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      placeholder="Ceritakan lebih detail tentang karyamu..."
                      required
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      Unggah Gambar Karya (Maks. 5, @5MB/gambar){" "}
                      ...
                      {images.length > 0 && (
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 mb-4">
                          {images.map((img, index) => (
                            <div
                              key={index}
                              className="relative group aspect-w-1 aspect-h-1 bg-gray-200 rounded-md overflow-hidden shadow-sm"
                            >
                              <Image
