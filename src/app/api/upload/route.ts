...
export async function POST(request: NextRequest) {
  try {
    ...
    if (file.size > MAX_FILE_SIZE_MB * 1024 * 1024) {
      return NextResponse.json(
        { error: `Ukuran file terlalu besar. Maksimal ${MAX_FILE_SIZE_MB}MB.` },
        { status: 413 },
      );
    }
    ...
    const fileExt =
      ...
      (fileExtensionFromLib ? `.${fileExtensionFromLib}` : "");
    const originalNameBase = path.basename(file.name, path.extname(file.name));
    const safeOriginalNameBase = originalNameBase.replace(
      /[^a-zA-Z0-9_.-]/g,
      "_",
    );
    ...
    if (error.code === "EACCES") {
      errorMessage =
        "Kesalahan izin: Tidak dapat menulis file ke direktori uploads.";
    }
