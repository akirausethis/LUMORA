...
import {
  PackageIcon,
  UserIcon as SellerIcon,
  ShoppingBagIcon as BuyerIcon,
  CalendarIcon,
  TagIcon,
  DollarSignIcon,
  EyeIcon,
...
type Order = {
  ...
  customerName: string;
  customerEmail: string; // Idealnya ada customerId
  orderDate: string;
...
const OrdersPage = () => {
      ...
      try {
        ...
        if (storedOrders) {
          ...
          if (user.role === "buyer") {
            ...
            userSpecificOrders = allOrders.filter(
              (order) =>
            ...
            ); // Asumsi customerEmail sebagai identifier, idealnya customerId
        ...
        setModalState({
          isOpen: true,
          title: "Error Data",
          message: "Gagal memuat riwayat pesanan Anda.",
          type: "error",
  ...
  const getStatusText = (status: Order["status"] | string): string => {
    // Izinkan string juga untuk fallback
    switch (status) {
      case "paid":
  ...
  if (!currentUser) {
    // Jika tidak ada user (sudah dihandle modal, tapi sebagai fallback)
    return null;
  }
            ...
            <div className="text-center py-16 bg-white rounded-2xl shadow-lg border border-gray-200/70">
              ...
              <p className="text-xl font-semibold text-gray-700">
                {filterStatus === "all"
                  ...
                  : `Tidak ada pesanan dengan status "${getStatusText(filterStatus)}".`}
              </p>
              {currentUser.role === "buyer" && (
                <p className="mt-2 text-gray-500">
                  Mulai jelajahi layanan terbaik di Lumora!
                </p>
              )}
              {currentUser.role === "seller" && (
                <p className="mt-2 text-gray-500">
                  Tunggu pesanan pertama Anda masuk!
                </p>
              )}
              {currentUser.role === "buyer" && (
                <Link
              ...
              {filteredOrders.map((order) => {
                ...
                if (currentUser.role === "buyer" && mainItem?.sellerId) {
                  ...
                  counterPartyRoleIcon = (
                    <SellerIcon size={14} className="mr-1 text-gray-400" />
                  );
                } else if (currentUser.role === "seller") {
                  counterPartyName = order.customerName;
                  counterPartyRoleIcon = (
                    <BuyerIcon size={14} className="mr-1 text-gray-400" />
                  );
                }
                ...
                return (
                  <div
                    key={order.orderId}
                    className="bg-white shadow-lg hover:shadow-xl transition-all duration-300 rounded-xl border border-gray-200/80 overflow-hidden"
                  >
                    <div className="p-5 sm:p-6">
                      <div className="flex flex-col sm:flex-row justify-between items-start gap-3 mb-3">
                        <div>
                          <Link
                            href={`/progress/${order.orderId}`}
                        ...
                        <span
                          className={`text-xs font-semibold px-3 py-1 rounded-full whitespace-nowrap ${getStatusColor(order.status)}`}
                        >
