...
export type Order = {
  // Diekspor agar bisa diimpor di ProgressPage jika perlu
  orderId: string;
...
const PaymentPage = () => {
  ...
  const [modalState, setModalState] =
    useState<ModalStateType>(initialModalState);
  ...
  const paymentMethods = [
    {
      id: "qris_generic",
      name: "Scan QR (Semua E-Wallet & M-Banking)",
      icon: <QrcodeIcon className="w-6 h-6 mr-2 text-emerald-600" />,
    },
    {
      id: "bank_transfer_bca",
      name: "Bank Transfer (Virtual Account BCA)",
      icon: (
        <Image
          src="/bca.webp"
          alt="BCA"
    ...
    },
    {
      ...
      name: "Bank Transfer (Virtual Account Mandiri)",
      icon: (
        <Image
          src="/mandiri.webp"
          alt="Mandiri"
    ...
    },
    {
      ...
      icon: <CreditCardIcon className="w-5 h-5 mr-2 text-sky-600" />,
    },
  ];
  ...
  useEffect(() => {
    ...
    if (storedCurrentUser) {
      try {
        const currentUser: { fullName?: string; email?: string } =
        ...
        console.warn("Gagal parse currentUser:", e);
      }
    }

    if (transactionId) {
      const storedCheckoutCart = localStorage.getItem(
        `checkoutCart_${transactionId}`,
      );
      if (storedCheckoutCart) {
        try {
          ...
          if (validatedCart.length === 0) {
            setModalState({
              ...
              message: "Keranjang pembayaran Anda kosong.",
              type: "warning",
              buttons: [
                {
                  text: "Ke Keranjang",
          ...
          setModalState({
            isOpen: true,
            title: "Error Data Pembayaran",
            message:
              e.message ||
              "Gagal memuat detail pembayaran. Silakan coba dari keranjang.",
            type: "error",
            buttons: [
              {
                text: "Ke Keranjang",
        ...
        setModalState({
          ...
          message:
            "Detail pembayaran tidak ada atau sudah diproses. Kembali ke keranjang.",
          type: "warning",
          buttons: [
            {
              text: "Ke Keranjang",
  ...
  const confirmAndSaveOrder = async (orderToSave: Order) => {
    ...
    try {
      ...
      localStorage.setItem(
        "userOrders",
        JSON.stringify([...existingOrders, orderToSave]),
      );
      ...
      setModalState({
        ...
        message: `Pesanan Anda #${orderToSave.orderId} dengan metode ${orderToSave.paymentMethod} telah diterima.\nAnda akan diarahkan ke halaman progres.`,
        type: "success",
        buttons: [
          {
            text: "Lihat Progres Pesanan",
            onClick: () => {
              closeGenericModal();
              router.push(`/progress/${orderToSave.orderId}`);
            },
          },
        ],
        onClose: () => {
      ...
      setModalState({
        ...
        title: "Error Sistem",
        message:
          "Terjadi kesalahan saat memproses pesanan Anda setelah konfirmasi.",
        type: "error",
        buttons: [{ text: "Tutup", onClick: closeGenericModal }],
  ...
  const handleProcessPayment = async () => {
    if (!customerName.trim() || !customerEmail.trim()) {
      setModalState({
        ...
        message: "Harap isi Nama Lengkap dan Alamat Email Anda.",
        type: "warning",
        buttons: [{ text: "Mengerti", onClick: closeGenericModal }],
        onClose: closeGenericModal,
      });
      return;
    }
    ...
    if (!selectedPaymentMethod) {
      setModalState({
        ...
        message: "Harap pilih salah satu metode pembayaran.",
        type: "warning",
        buttons: [{ text: "Mengerti", onClick: closeGenericModal }],
        onClose: closeGenericModal,
      });
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-100">
        ...
        <p className="ml-3 text-gray-700">Memuat detail pembayaran...</p>{" "}
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-gray-100 flex flex-col justify-center items-center p-6">
        ...
        <h2 className="text-2xl font-semibold text-gray-700 mb-3">
          Keranjang Pembayaran Kosong
        </h2>{" "}
        <p className="text-gray-500 mb-8 text-center max-w-md">
          {" "}
          Tidak ada item untuk pembayaran ini atau transaksi mungkin sudah
          selesai.{" "}
        </p>{" "}
        <button
          onClick={() => router.push("/explore")}
          className="bg-emerald-600 text-white py-3 px-8 rounded-lg hover:bg-emerald-700 transition duration-300 ease-in-out shadow-md hover:shadow-lg"
        >
          {" "}
          Lanjutkan Belanja{" "}
        </button>{" "}
          ...
          <div className="bg-white p-6 sm:p-8 rounded-xl shadow-2xl max-w-md w-full text-center transform transition-all scale-100 opacity-100 animate-modalShow">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-semibold text-gray-800 text-left">
                Bayar dengan {qrPaymentModal.paymentMethodName}
              </h3>
              <button
            ...
            <p className="text-gray-600 text-sm mb-2">
              Silakan pindai QR Code di bawah ini untuk menyelesaikan pembayaran
              Anda.
            </p>
            <div className="my-6 flex justify-center">
              <Image
                src={qrPaymentModal.qrCodeUrl}
                alt="QR Code Pembayaran"
                width={280}
                height={280}
                className="border-4 border-emerald-200 rounded-lg shadow-sm p-1 bg-white"
              />
            </div>
            ...
            <p className="text-3xl font-bold text-emerald-600 mb-6">
              {formatPrice(qrPaymentModal.amount)}
            </p>
            <p className="text-xs text-gray-500 mb-3">
              Setelah melakukan pembayaran, klik tombol konfirmasi di bawah ini.
            </p>
            <button
              onClick={() =>
                qrPaymentModal.orderToProceed &&
                confirmAndSaveOrder(qrPaymentModal.orderToProceed)
              }
            ...
            <p className="text-xs text-gray-400 mt-4 italic">
              Ini adalah simulasi pembayaran. Tidak ada transaksi nyata yang
              terjadi.
            </p>
          </div>
        </div>
        ...
        <div className="min-h-screen bg-gray-50 py-8 sm:py-12 px-4 selection:bg-emerald-500 selection:text-white">
          <div className="max-w-4xl mx-auto">
            <button
              ...
              className="flex items-center text-emerald-600 hover:text-emerald-700 mb-6 group text-sm font-medium"
            >
              {" "}
              <SolidArrowLeftIcon className="w-5 h-5 mr-1.5 transition-transform group-hover:-translate-x-1" />{" "}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 lg:gap-8 items-start">
              <div className="lg:col-span-3 space-y-6">
                <div className="bg-white p-6 rounded-xl shadow-lg">
                  <h2 className="text-xl font-semibold text-gray-800 mb-5 border-b border-gray-200 pb-3">
                    Detail Pesanan Anda
                  </h2>
                  {checkoutItems.map((item) => (
                    <div
                      key={item.id}
                      className="flex items-start gap-4 pt-1 pb-4 border-b border-gray-100 last:border-b-0 last:pb-0"
                    >
                      <div className="w-16 h-16 sm:w-20 sm:h-20 flex-shrink-0 relative rounded-md overflow-hidden border border-gray-200 bg-gray-50">
                        {item.image ? (
                          ...
                          <ShoppingBagIcon className="w-full h-full text-gray-300 p-2" />
                        )}
                      </div>
                      <div className="flex-grow">
                        {" "}
                        <h3 className="text-sm sm:text-md font-semibold text-gray-700 leading-tight line-clamp-2">
                          {item.title}
                        </h3>{" "}
                        <p className="text-gray-500 text-xs sm:text-sm">
                          Kuantitas: {item.quantity}
                        </p>{" "}
                        <p className="text-emerald-600 font-semibold mt-0.5 text-sm sm:text-base">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <div className="relative">
                        {" "}
                        <UserIcon className="w-4 h-4 text-gray-400 absolute left-3 top-1/2 transform -translate-y-1/2 pointer-events-none" />{" "}
                        <input
                          type="text"
                          id="customerName"
                          value={customerName}
                          ...
                          className="w-full text-sm pl-9 pr-3 py-2.5 border border-gray-300 rounded-lg focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 transition placeholder-gray-400"
                        />{" "}
                      </div>{" "}
                    </div>
                    <div>
                      ...
                      <div className="relative">
                        {" "}
                        <MailIcon className="w-4 h-4 text-gray-400 absolute left-3 top-1/2 transform -translate-y-1/2 pointer-events-none" />{" "}
                        <input
                          type="email"
                          id="customerEmail"
                          value={customerEmail}
                  ...
                  <div className="space-y-3">
                    {paymentMethods.map((method) => (
                      <label
                        ...
                        className={`flex items-center p-3.5 border rounded-lg cursor-pointer transition-all duration-200 ease-in-out hover:shadow-md hover:border-emerald-400 ${selectedPaymentMethod === method.id ? "border-emerald-500 ring-2 ring-emerald-500/70 bg-emerald-50/50" : "border-gray-200"}`}
                      >
                        <input
                          type="radio"
                ...
                <div className="bg-white p-6 rounded-xl shadow-lg">
                  ...
                  >
                    {isProcessingPayment ? (
                      ...
                      <div className="flex items-center justify-center">
                        {" "}
                        <CreditCardIcon className="w-5 h-5 mr-2" /> Bayar
                        Sekarang{" "}
                      </div>
                    )}
                  </button>
                  {transactionId && (
                    <p className="text-xs text-gray-400 mt-3 text-center">
                      ID Transaksi:{" "}
                      <span className="font-mono">{String(transactionId)}</span>
                    </p>
                  )}
                </div>
              </div>
