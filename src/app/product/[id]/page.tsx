...
import {
  ...
  Package,
  HelpCircle,
  ShoppingCartIcon as LucideShoppingCartIcon, // Menggunakan ikon keranjang dari Lucide
} from "lucide-react";
            ...
            >
              <Image
                ...
                onError={(e: React.SyntheticEvent<HTMLImageElement, Event>) => {
                  ...
                  (e.target as HTMLImageElement).alt = "Gagal memuat thumbnail";
                }}
              />
            </div>
          ))}
...
const SinglePage = () => {
  ...
  useEffect(() => {
    ...
    if (!id) {
      setModalState({
        isOpen: true,
        title: "Error",
        message: "Product ID tidak valid atau tidak tersedia.",
        type: "error",
        buttons: [
          {
            text: "OK",
    ...
    if (storedProducts) {
      ...
      setModalState({
        ...
        message: "Tidak ada data produk di penyimpanan lokal.",
        type: "error",
        buttons: [{ text: "OK", onClick: closeGenericModal }],
        onClose: closeGenericModal,
      });
  ...
  const handleAddToCart = (itemToAdd: Product, qty: number = 1) => {
    ...
    if (existingItemIndex > -1) {
      ...
      const priceAsNumber =
        typeof itemToAdd.price === "string"
          ? parseFloat(itemToAdd.price.replace(/\D/g, ""))
          : itemToAdd.price;
  ...
  const handleBuyNow = () => {
    if (!product) {
      setModalState({
        isOpen: true,
        title: "Gagal",
        message: "Produk tidak valid untuk dibeli.",
        type: "error",
        buttons: [{ text: "OK", onClick: closeGenericModal }],
        onClose: closeGenericModal,
      });
    ...
    const itemToCheckout: CartItem = {
      ...product,
      price: priceAsNumber,
      quantity: quantity,
    }; // Gunakan state quantity
  ...
  if (!product && !modalState.isOpen) {
    /* ... (JSX produk tidak ditemukan) ... */
  }
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative py-10 sm:py-12 bg-white my-6 sm:my-8 rounded-xl shadow-2xl">
        <div className="flex flex-col lg:flex-row gap-10 lg:gap-16">
          ...
          <div className="w-full lg:w-1/2 flex flex-col gap-5">
            ...
            <div className="flex items-center gap-4 text-sm text-gray-600 border-t border-b border-gray-100 py-3 my-2">
              {" "}
              <div className="flex items-center gap-1">
                {" "}
                <Star className="w-4 h-4 text-yellow-400 fill-yellow-400" />{" "}
                <span className="font-semibold">4.8</span>{" "}
                <span className="text-gray-500">(251 ulasan)</span>{" "}
              </div>{" "}
            ...
            <div className="text-sm text-gray-600 flex flex-wrap gap-x-6 gap-y-3 bg-gray-50 p-4 rounded-lg border border-gray-200">
              {" "}
              <p>
                {" "}
            ...
            <div className="space-y-5 mt-3">
              ...
              {product.deliveryTime && (
                <div className="flex items-center gap-3 p-3 bg-blue-50/70 rounded-lg border border-blue-200">
                  {" "}
                  ...
                  <div>
                    ...
                    <p className="text-gray-600 text-sm">
                      {product.deliveryTime} hari kerja
                    </p>{" "}
                  </div>{" "}
                </div>
              )}
              {product.revisions !== undefined && (
                <div className="flex items-center gap-3 p-3 bg-green-50/70 rounded-lg border border-green-200">
                  {" "}
                  <Repeat className="w-5 h-5 text-green-600 flex-shrink-0" />{" "}
                  <div>
                    {" "}
                    <h4 className="font-medium text-gray-800">
                      Jumlah Revisi
                    ...
                    <p className="text-gray-600 text-sm">
                      {" "}
                      {String(product.revisions).toLowerCase() === "unlimited"
                        ? "Tidak Terbatas"
                        : `${product.revisions} kali`}{" "}
                    </p>{" "}
                  </div>{" "}
                </div>
              )}
            ...
            {product.includedItems && product.includedItems.length > 0 && (
              <div className="mt-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
                {" "}
                <h4 className="font-semibold text-gray-800 flex items-center gap-2 mb-3 text-md">
                  {" "}
                  <Package className="w-5 h-5 text-gray-500" /> Apa Saja yang
                  Termasuk:{" "}
                </h4>{" "}
                <ul className="list-inside list-disc space-y-1.5 pl-2 text-sm text-gray-700">
                  {" "}
            ...
            {product.requirements && product.requirements.length > 0 && (
              <div className="mt-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
                ...
                <h4 className="font-semibold text-gray-800 flex items-center gap-2 mb-3 text-md">
                  {" "}
                  <HelpCircle className="w-5 h-5 text-gray-500" /> Yang
                ...
                <ul className="list-inside list-disc space-y-1.5 pl-2 text-sm text-gray-700">
                  ...
                  {product.requirements.map((req, index) => (
                    <li key={index} className="flex items-start">
                      {" "}
                      <MessageCircleMore className="w-4 h-4 text-purple-500 mr-2 mt-0.5 flex-shrink-0" />{" "}
                      <span>{req}</span>{" "}
                    </li>
                  ))}{" "}
                </ul>{" "}
            ...
            <div className="flex flex-col sm:flex-row gap-3 mt-6 pt-6 border-t border-gray-200">
              ...
              >
                <LucideShoppingCartIcon className="w-5 h-5" />{" "}
