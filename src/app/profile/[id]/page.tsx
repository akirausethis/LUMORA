...
const UserProfilePage = () => {
  ...
  useEffect(() => {
    ...
    if (storedCurrentUser) {
      try {
        setLoggedInUser(JSON.parse(storedCurrentUser));
      } catch (e) {
    ...
    if (storedUsers && userId) {
      try {
        ...
        if (foundUser) {
          ...
          setModalState({
            ...
            message: "Pengguna ini tidak ada.",
            type: "error",
            buttons: [
              {
                text: "OK",
      ...
      setModalState({
        ...
        type: "error",
        buttons: [{ text: "OK", onClick: () => router.push("/") }],
        onClose: () => router.push("/"),
      });
    }
    setIsLoading(false);
    ...
    if (e.target.files && e.target.files[0]) {
      ...
      if (!validImageTypes.includes(file.type)) {
        setModalState({
          ...
          message: "Hanya gambar (JPEG, PNG, GIF, WebP) yang diizinkan.",
          type: "warning",
          buttons: [{ text: "OK", onClick: closeGenericModal }],
          onClose: closeGenericModal,
        });
  ...
  const handleSaveChanges = async () => {
    ...
    if (selectedFile) {
      ...
      try {
        ...
        if (!response.ok) {
          /* ... (error handling fetch dengan setModalState) ... */
          const errorText = await response.text();
          setModalState({
            isOpen: true,
            title: "Upload Gagal",
            message: `Server: ${response.status}. ${errorText.substring(0, 100)}`,
            type: "error",
            buttons: [{ text: "OK", onClick: closeGenericModal }],
            onClose: closeGenericModal,
          });
        ...
        if (!contentType || !contentType.includes("application/json")) {
          /* ... (error handling non-JSON dengan setModalState) ... */
          const responseText = await response.text();
          setModalState({
            isOpen: true,
            title: "Respons API Salah",
            message: `Format respons tidak benar. Diterima: ${contentType}.`,
            type: "error",
            buttons: [{ text: "OK", onClick: closeGenericModal }],
            onClose: closeGenericModal,
          });
        ...
        if (uploadResult.fileUrl) {
          ...
          throw new Error(uploadResult.error || "URL file tidak ada dari API.");
        }
      } catch (error: any) {
        setModalState({
          isOpen: true,
          title: "Upload Foto Gagal",
          message: error.message || "Tidak dapat mengunggah foto profil.",
          type: "error",
          buttons: [{ text: "OK", onClick: closeGenericModal }],
          onClose: closeGenericModal,
        });
      ...
      try {
        ...
        if (userIndex !== -1) {
          const updatedUserDetails: UserData = {
            ...allUsers[userIndex],
            fullName:
              editingUser.fullName?.trim() || allUsers[userIndex].fullName,
            bio: editingUser.bio?.trim() || allUsers[userIndex].bio,
            phoneNumber:
              editingUser.phoneNumber?.trim() ||
              allUsers[userIndex].phoneNumber,
          ...
          if (loggedInUser && loggedInUser.id === userId) {
            ...
            localStorage.setItem(
              "currentUser",
              JSON.stringify(currentUserSafe),
            );
          ...
          setModalState({
            ...
            type: "success",
            buttons: [{ text: "Selesai", onClick: closeGenericModal }],
            onClose: closeGenericModal,
          });
          setIsEditMode(false);
        } else {
          setModalState({
            isOpen: true,
            title: "Kesalahan Update",
            message: "Pengguna tidak ditemukan untuk diupdate.",
            type: "error",
            buttons: [{ text: "OK", onClick: closeGenericModal }],
            onClose: closeGenericModal,
          });
        ...
        /* ... (setModalState untuk error localStorage) ... */
        setModalState({
          isOpen: true,
  ...
  if (isLoading) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-100">
        ...
        <p className="ml-3 text-gray-700">Memuat profil...</p>{" "}
      </div>
    );
  }
  if (!user) {
    /* ... (JSX user tidak ditemukan, modal sudah menghandle ini) ... */ return null;
  }
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="bg-white shadow-2xl rounded-2xl p-6 sm:p-8 md:p-10 relative">
            ...
            <div className="flex flex-col items-center sm:flex-row sm:items-start gap-6 sm:gap-8">
              <div className="relative group flex-shrink-0">
                ...
                {isEditMode && canEditProfile && (
                  <label
                    ...
                    className="absolute bottom-0 right-0 w-10 h-10 bg-emerald-500 text-white rounded-full flex items-center justify-center cursor-pointer shadow-md hover:bg-emerald-600 transition ring-2 ring-white"
              ...
              <div
                className={`text-center sm:text-left flex-grow mt-2 sm:mt-0 ${isEditMode ? "w-full" : ""}`}
              >
                {isEditMode && canEditProfile ? (
                  <div className="space-y-4">
                    ...
                    <p className="text-md text-emerald-700 bg-emerald-50 px-3 py-1.5 rounded-md inline-block">
                      @{user.username}{" "}
                  ...
                  <>
                    <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-800 mb-0.5">
                      {user.fullName || user.username}
                    </h1>
                    <p className="text-md sm:text-lg text-emerald-600 mb-3">
                      @{user.username}
                    </p>
                    <p className="text-sm sm:text-base text-gray-600 leading-relaxed max-w-lg">
                      {user.bio ||
                        (canEditProfile
                          ? "Bio Anda masih kosong. Klik tombol edit untuk menambahkan."
                          : "Pengguna ini belum menulis bio.")}
                    </p>
                    {user.phoneNumber && (
          ...
          {!isEditMode && (
            <div className="mt-8 bg-white shadow-xl rounded-2xl p-6 md:p-8">
              <h2 className="text-xl sm:text-2xl font-semibold text-gray-800 mb-6 border-b border-gray-200 pb-4">
