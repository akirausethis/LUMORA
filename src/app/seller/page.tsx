  ...
  useEffect(() => {
    ...
    if (storedUser) {
      try {
        ...
        setModalState({
          ...
          message:
            "Terjadi kesalahan saat memuat data sesi Anda. Silakan login kembali.",
          type: "error",
          buttons: [
            {
              text: "Login",
    ...
    if (user && user.role !== "seller") {
      setModalState({
        ...
        title: "Akses Ditolak",
        message:
          "Halaman ini hanya untuk penjual. Apakah Anda ingin mendaftar sebagai penjual?",
        type: "confirmation",
        buttons: [
          {
            text: "Tidak, Kembali",
          ...
          {
            text: "Ya, Daftar Jadi Penjual",
            onClick: () => {
              closeGenericModal();
              if (user) router.push(`/setseller/${user.id}`);
            },
          },
        ],
        onClose: () => {
    ...
    if (savedProductsString && user) {
      try {
        ...
        setModalState({
          ...
          message: "Gagal memuat daftar produk Anda.",
          type: "error",
      ...
      if (storedOrdersString) {
        try {
          ...
          allOrders.forEach((order) => {
            const sellerItemsInOrder = order.items.filter(
            ...
            if (sellerItemsInOrder.length > 0) {
              ...
              if (order.status === "completed") {
                const salesFromThisOrder = sellerItemsInOrder.reduce(
          ...
          console.error("Gagal parse userOrders untuk statistik:", e);
        }
      }
  ...
  const stats = [
    {
      ...
      value: products.filter((p) => p.status !== "archived").length.toString(),
      icon: ArchiveIcon,
      color: "text-blue-500",
      bgColor: "bg-blue-50",
    },
    {
      name: "Total Penjualan Selesai",
      value: formatPriceForStat(totalSales),
      icon: DollarSignIcon,
        ...
        <main className="max-w-7xl mx-auto py-10 sm:py-12 px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-8 md:mb-10">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 p-6 bg-white rounded-2xl shadow-xl border border-gray-200/80">
              <div className="flex items-center gap-4">
                <Image
                  src={
                    currentUser?.profilePictureUrl || "/profile-placeholder.png"
                  }
                  alt="Foto Profil Penjual"
                  width={64}
                  height={64}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 mb-8 md:mb-10">
            {stats.map((stat) => (
              <div
                key={stat.name}
                className={`p-5 rounded-xl shadow-lg flex items-center gap-4 ${stat.bgColor} border border-gray-200/70`}
              >
                <div
                  className={`p-3 rounded-full bg-white shadow-sm ${stat.color}`}
                >
                  {" "}
          ...
          <div className="bg-white rounded-2xl shadow-xl p-6 md:p-8">
            ...
            {products.length === 0 ? (
              <div className="text-center py-16 border-2 border-dashed border-gray-200 rounded-xl bg-gray-50/50">
                ...
                <Link
                  href="/addproduct"
                  className="inline-flex items-center gap-2 px-8 py-3 text-base font-semibold rounded-xl shadow-md text-white bg-emerald-500 hover:bg-emerald-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500 transition-transform transform hover:scale-105"
                >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {products.map((product) => (
                  <div
                    key={product.id}
                    className="bg-white border border-gray-200/80 rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 group flex flex-col overflow-hidden transform hover:-translate-y-1"
                  >
                    <div className="relative w-full h-52 bg-gray-100 flex-shrink-0">
                      ...
                      {product.status && product.status !== "active" && (
                        <span
                          className={`absolute top-2 right-2 text-xs font-semibold px-2 py-1 rounded-full shadow-sm ${product.status === "draft" ? "bg-yellow-100 text-yellow-700 border border-yellow-300" : "bg-gray-100 text-gray-600 border border-gray-300"}`}
                        >
                          {" "}
                    ...
                    <div className="p-4 flex flex-col flex-grow">
                      <h3
                        className="text-md font-semibold text-gray-800 mb-1.5 group-hover:text-emerald-600 transition-colors duration-200 line-clamp-2 leading-tight h-12 cursor-pointer"
                        onClick={() =>
                      ...
                      <div className="flex items-center text-xs mb-2.5">
                        {" "}
                        <span
                          className="bg-emerald-50 text-emerald-700 px-2 py-0.5 rounded-full font-medium line-clamp-1"
                      ...
                      <div className="mt-auto pt-3 border-t border-gray-100">
                        <p className="text-xl font-bold text-emerald-600 mb-3">
                          {" "}
                          {formatPriceForProduct(product.price)}{" "}
                        </p>
