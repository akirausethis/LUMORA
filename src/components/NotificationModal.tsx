...
const NotificationModal: React.FC<NotificationModalProps> = ({
  ...
  const modalIcon =
    ...
    (DefaultIcon ? (
      <DefaultIcon
        className={`w-12 h-12 sm:w-16 sm:h-16 ${iconColorClass} mx-auto mb-4`}
      />
    ) : null);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 backdrop-blur-sm flex items-center justify-center z-[9999] p-4 transition-opacity duration-300 ease-in-out">
      ...
      >
        ...
        <div className="text-center">
          ...
          <h3
            id="modal-title"
            className={`text-xl sm:text-2xl font-semibold ${titleColorClass} mb-2.5`}
          >
            {title}
          </h3>
          <div
            id="modal-message"
            className="text-gray-600 text-sm sm:text-base mb-7 whitespace-pre-line"
          >
            {message}
          </div>
        </div>
        {buttons && buttons.length > 0 && (
          <div
            className={`flex gap-3 ${buttons.length === 1 || (buttons.length > 1 && !buttons.some((b) => b.className?.includes("bg-gray-200"))) ? "justify-center" : "flex-col sm:flex-row"}`}
          >
